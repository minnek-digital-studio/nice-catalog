...
export default function PlansSettings() {
    ...
    useEffect(() => {
        ...
        if (params?.success === 'true') {
            finishCheckout(params.session_id, params.plan_id, params.customer_id);
        }
    ...
    const finishCheckout = async (sessionId: string, planId: string, customerId: string) => {
        try {
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                {plans.map((plan) => {
                    ...
                    return (
                        <div
                            key={plan.id}
                            className={`relative rounded-lg border ${isCurrentPlan ? 'border-[#ed1c24] ring-2 ring-[#ed1c24]' : 'border-gray-200'
